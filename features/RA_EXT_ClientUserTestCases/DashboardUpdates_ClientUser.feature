...
Feature: Dashboard Updates for Client User
  ...
  Scenario: TC006_02_ClientUser_DashboardUpdates - Apply a predefined filter
    ...
    When select "28 Nov 2025" in the "Start date" filter on the Dashboard filter
  ...
  Scenario: TC006_04_ClientUser_DashboardUpdates - Apply a customized filter
    ...
    And save the "ClientPortal_20260209133616 - Updates Dashboard" items
    When press "Filter" button on the Dashboard filter
    And select "Test_ClientUser" located in the "Saved Filters" section on the Dashboard filter
    When press "View results" button on the Dashboard filter
    Then verify the "ClientPortal_20260209133616 - Updates Dashboard" item count is not the same
