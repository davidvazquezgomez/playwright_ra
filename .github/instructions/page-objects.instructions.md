...
- When a business action is only available to a subset of roles, call `BasePage.requireAdministratorRole(action)` as the first statement of the owning page-object method. It reads the role recorded for the current browser page and throws a technical error when the role does not contain `ADMIN`, or when no role was recorded. Use it for administrator-only actions such as `ClientPortalListPage.editFirstClientPortal` and `ClientPortalListPage.editClientPortal`.
- The authenticated role is stored per browser page by `BasePage.recordAuthenticatedRole(role)`, which the login step calls. Read it with `BasePage.getAuthenticatedRole()` when it adds useful evidence to a failure message; do not re-read credentials or environment variables in a page object to infer the role.
- When an element is known to render only after a page refresh, wrap the detection in `BasePage.retryWithReload(condition, attempts)` instead of adding fixed waits or ad hoc reload loops. Put the full re-setup inside the condition, including re-navigating and re-applying grid filters, because the reload discards that state.
- `retryWithReload` retries errors thrown by the condition and re-throws them on the last attempt, so technical failures stay native. Report a confirmed application defect with `failWithApplicationError` only after `retryWithReload` returns `false`.
